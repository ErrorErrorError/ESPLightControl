...
public class JellyToggleButton extends CompoundButton {
    ...
    private static final boolean DEFAULT_THUMB_SHADOW = false;
    private static final float DEFAULT_THUMB_SHADOW_RADIUS_DP = 5;
    private static final float DEFAULT_THUMBS_SHADOW_PADDING_VALUE = 8;
    ...
    private boolean mThumbShadow = DEFAULT_THUMB_SHADOW;
    private float mThumbShadowRadius;
    private float mThumbShadowPadding;
    ...
    private void init(@Nullable AttributeSet attrs) {
        ...
        mThumbShadowRadius = DEFAULT_THUMB_SHADOW_RADIUS_DP * density;
        mThumbShadowPadding = DEFAULT_THUMBS_SHADOW_PADDING_VALUE * density;
        ...
        if (ta != null) {
            ...
            mThumbShadow = ta.getBoolean(R.styleable.JellyToggleButton_jtbThumbShadow, DEFAULT_THUMB_SHADOW);
            mThumbShadowRadius = ta.getDimension(R.styleable.JellyToggleButton_jtbThumbShadowRadius, DEFAULT_THUMB_SHADOW_RADIUS_DP);
...
//    }
    ...
    protected void onDraw(@NonNull Canvas canvas) {
        ...
        if (mThumbShadow) {
            //Needs to be a padding for the shadow
            setPadding((int) mThumbShadowPadding, (int) mThumbShadowPadding, (int) mThumbShadowPadding, (int) mThumbShadowPadding);
            mPaint.setShadowLayer(mThumbShadowRadius, 0, 5, Color.rgb(187, 187, 187)); // This set's color shadow to grey
            canvas.drawPath(mThumbPath, mPaint);
            mPaint.clearShadowLayer(); //We only want shadow in the thumb, not everything else.
        } else {
            canvas.drawPath(mThumbPath, mPaint);
        }
    ...
    public float getThumbShadowRadius() {
        return mThumbShadowRadius;
    }

    public void setThumbShadowRadius(float radius) {
        this.mThumbShadowRadius = radius;
    }

    public boolean isThumbShadow() {
        return mThumbShadow;
    }

    public void setThumbShadow(boolean shadow) {
        this.mThumbShadow = shadow;
    }
