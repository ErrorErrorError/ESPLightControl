...
abstract class ScrollingController implements IScrollingController {
    ...
    private int computeScrollOffset(@NonNull RecyclerView.State state) {
    ...
    private int computeScrollExtent(@NonNull RecyclerView.State state) {
        if (lm.getChildCount() == 0 || state.getItemCount() == 0) {
            return 0;
        }
    ...
    private int computeScrollRange(@NonNull RecyclerView.State state) {
        if (lm.getChildCount() == 0 || state.getItemCount() == 0) {
            return 0;
        }
